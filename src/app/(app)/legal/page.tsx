...
export default function LegalPage() {
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      {/* ── Disclaimer banner ────────────────────────────────────────── */}
      <div className="flex gap-3 rounded-xl bg-aura-surface border border-aura-border p-4 mb-8">
        <Shield size={20} className="text-aura-warning flex-shrink-0 mt-0.5" />
      ...
      <div className="space-y-4">
        {LEGAL_REFERENCES.map((ref) => {
          ...
          return (
            <div
              key={ref.key}
              className="rounded-xl bg-aura-surface border border-aura-border p-5 transition-colors hover:border-aura-border"
            >
              {/* Top row: Norwegian name + staleness badge */}
              <div className="flex items-start justify-between gap-3 mb-1">
                <div className="flex items-center gap-2">
                  <Scale size={16} className="text-aura-primary flex-shrink-0" />
                ...
                {stale && (
                  <span className="flex items-center gap-1 text-[10px] text-aura-warning bg-aura-warning/10 rounded-full px-2 py-0.5 flex-shrink-0">
              ...
              {/* English name */}
              <p className="text-xs text-aura-text-secondary mb-3 ml-6">{ref.nameen}</p>

              {/* Relevance description */}
              <p className="text-sm text-aura-text-secondary leading-relaxed mb-3">
                {ref.relevance}
              </p>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <a
                  ...
                  className="flex items-center gap-1 text-aura-primary hover:text-aura-primary-light transition-colors"
                >
                  Read on Lovdata
      ...
      <div className="mt-12">
        <h2 className="font-display text-2xl text-aura-text mb-4">
          Privacy &amp; Terms
        </h2>

        <div className="rounded-xl bg-aura-surface border border-aura-border p-5 space-y-4">
          <div>
            <h3 className="text-sm font-medium text-aura-text mb-1">
              How Aura handles your data
            </h3>
            <ul className="text-xs text-aura-text-secondary leading-relaxed space-y-1.5 list-disc list-inside">
              <li>
